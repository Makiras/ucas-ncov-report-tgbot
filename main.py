...
def start_entry(update, context):
    ...
    update.message.reply_text("Welcome, {}. try /help.\nSpecial Thanks to https://github.com/HenryzhaoH/bupt-ncov-report-tgbot and https://github.com/WANNA959/ucas-covid19 ".format(update.message.from_user.username or update.message.from_user.first_name or ''), disable_web_page_preview=True)
...
def list_entry(update, context, admin_all=False):
    ...
    if admin_all == True:
        users = UCASUser.select().where(UCASUser.status != UCASUserStatus.removed).prefetch(TGUser)
    else:
        # users = UCASUser.select().where(UCASUser.owner == update.message.from_user.id).order_by(UCASUser.id.asc())
        tguser = TGUser.get(
            userid = update.message.from_user.id
        )
        users = tguser.ucasusers.where(UCASUser.status != UCASUserStatus.removed)
    ret_msgs = []
    ret_msg = ''
